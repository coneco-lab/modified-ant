...
The timeline of this experiment (i.e., the time intervals between experimental events) is optimised for mANT-EEG experiments. This is because this code was developed for a behavioural (i.e., pure mANT) experiment that served as pilot for a subsequent mANT-EEG experiment. 
If you want, you can still use the same timeline as Fan et al. (2002): just change the appropriate parameters in `config.py`. However, note that Fan et al.'s results can be reproduced with the current timeline.  
